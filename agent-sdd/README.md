...
│   │       ├── lint-gate.sh
│   │       └── done-gate.sh
│   ├── spec-kit/                   ← blank spec-kit templates (wizard fills these into agent-artifacts/)
...
- `agent-artifacts/` — your project's generated SDD data. Committed alongside source code. Created automatically by the wizard.
...
On day 1, `agent-artifacts/context/` is empty. The agent needs a context file
...
- **Skeleton update available** — run `git submodule update --remote agent-sdd` to pull the latest tool version. Your `agent-artifacts/` data is unaffected.
